...
import { Order, Customer, CompanyInfo } from '../types';
import { 
...
const DeliveryChallan: React.FC<DeliveryChallanProps> = ({ 
  orders, customers, onAddChallan, onUpdateChallan, onAction, currency = '₹', 
  ...
  const generateDCPDF = (challan: any) => {
    ...
    doc.setTextColor(0);
    doc.setFont("helvetica", "bold");
    doc.text(companyInfo.name.toUpperCase(), pageWidth - 15, 20, { align: 'right' });
    doc.setFont("helvetica", "normal");
    doc.text(companyInfo.address || "", pageWidth - 15, 25, { align: 'right' });
    ...
    doc.setFont("helvetica", "normal");
    doc.text(challan.customerName, 15, 55);

    doc.setFont("helvetica", "bold");
    doc.text("Transporter:", 120, 50);
    doc.setFont("helvetica", "normal");
    doc.text(challan.transportName || '', 120, 55);
