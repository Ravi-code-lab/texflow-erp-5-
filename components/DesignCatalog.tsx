...
const DesignCatalog: React.FC<DesignCatalogProps> = ({ designs, inventory, onAdd, onUpdate, onDelete, currency = '₹' }) => {
  ...
  const handleSave = () => {
    if (!form.name) return;
    const d = { ...form, processCostPerPiece: cost.landed, id: form.id || `ITM-${Date.now().toString().slice(-6)}`, updatedAt: new Date().toISOString() } as Design;
            ...
            <Card title="Image Gallery" sub="Additional product images"
              ...
              <div className="flex gap-3 flex-wrap">
                {(form.altImages || []).map((url: string, i: number) => (
                  <div key={i} className="relative group/img w-20 h-20 rounded-md border border-[#e1e8ed] overflow-hidden bg-[#f7f9fb]">
                ...
                <table className="w-full text-left min-w-[600px]">
                  ...
                  <tbody>
                    {(form.recipe || []).map((item: RecipeItem, idx: number) => {
                      ...
                      return (
                        <tr key={idx} className="border-b border-[#f0f2f5] hover:bg-[#f7f9fb]">
