...
import { 
  ...
  StockAudit, PayrollAdjustment, SampleRequest, Pack, StockTransfer, WasteLog, BrokerageLog, MarginCosting,
...
import { getItem, setItem, hydrateFromNative, getVaultSnapshot } from './utils/networkClient';
import { Loader2, Command, Menu, Search, Bell, CheckCircle2, AlertCircle, Info, XCircle, Check, Clock, ExternalLink, BellOff, X } from 'lucide-react';
...
const ForcePasswordChangeModal: React.FC<{ onSave: (pw: string) => Promise<void> }> = ({ onSave }) => {
  ...
  const handleSave = async () => {
    ...
    if (pw === 'admin123')          { setErr('Please choose a different password from the default.'); return; }
    setSaving(true); setErr('');
    try { await onSave(pw); } catch { setErr('Failed to save. Please try again.'); setSaving(false); }
  };
      ...
      <div className="w-full max-w-sm mx-4 bg-white dark:bg-slate-900 rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-700 p-7">
        <div className="flex items-center gap-3 mb-5">
          <div className="w-10 h-10 rounded-xl bg-amber-100 dark:bg-amber-900/40 flex items-center justify-center">
            <svg className="w-5 h-5 text-amber-600 dark:text-amber-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M12 9v3m0 4h.01M10.29 3.86L1.82 18a2 2 0 001.71 3h16.94a2 2 0 001.71-3L13.71 3.86a2 2 0 00-3.42 0z"/></svg>
          </div>
          <div>
            <p className="font-bold text-slate-900 dark:text-white text-base">Set Admin Password</p>
            <p className="text-xs text-slate-500 dark:text-slate-400">First-time setup — required before continuing</p>
          </div>
        </div>

        <p className="text-xs text-slate-500 dark:text-slate-400 mb-5 leading-relaxed">
          You're logged in with the default seed credentials. Please create a secure password for the <strong>admin</strong> account before proceeding.
        </p>

        <div className="space-y-3">
          <div>
            <label className="text-xs font-semibold text-slate-600 dark:text-slate-300 uppercase tracking-wider mb-1 block">New Password</label>
            <div className="relative">
              ...
              />
              <button type="button" onClick={() => setShow(s => !s)} className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400 hover:text-slate-600 dark:hover:text-white">
                {show
          ...
          <div>
            <label className="text-xs font-semibold text-slate-600 dark:text-slate-300 uppercase tracking-wider mb-1 block">Confirm Password</label>
            <input
              type={show ? 'text' : 'password'}
              value={pw2}
        ...
        >
          {saving
            ...
            : <><svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7"/></svg> Save Password & Continue</>
          }
        </button>
      </div>
    </div>
...
const App: React.FC = () => {
  ...
  const refreshData = useCallback(async () => {
    ...
    try {
        ...
        setInvoiceConfig(await g<InvoiceConfig>('texflow_invoice_config', invoiceConfig));
        setShopifyConfig(await g<ShopifyConfig>('texflow_shopify_config', shopifyConfig));
        setSecurityConfig(await g<SecurityConfig>('texflow_security_config', securityConfig));
        setCommunicationConfig(await g<CommunicationConfig>('texflow_communication_config', communicationConfig));
        setAdvancedConfig(await g<AdvancedConfig>('texflow_advanced_config', advancedConfig));
        ...
        setMachines(await g<Machine[]>('machines', [
          {
            id: 'MAC-JET-A',
        ...
        setSupplierQuotations(await g<SupplierQuotation[]>('supplierQuotations', []));
        ...
        setYarnLots(await g<YarnLot[]>('yarnLots', [
          {
        ...
        setDyeingJobs(await g<DyeingJob[]>('dyeingJobs', [
          {
            id: 'DYE-101',
        ...
        setFabricCostings(await g<FabricCosting[]>('fabricCostings', [
          {
            id: 'FC-101',
        ...
        setDispatchEntries(await g<DispatchEntry[]>('dispatchEntries', [
          {
            id: 'DISP-101',
        ...
        setPayrollAdjustments(await g<Record<string, PayrollAdjustment>>('payrollAdjustments', {}));
        setDynamicDocuments(await g<Record<string, any[]>>('dynamicDocuments', {}));
  ...
  const marginCostingMgr = handleCollection('marginCostings', marginCostings, setMarginCostings);
  ...
  if (!isAuthenticated) {
    ...
    const handleLogin = async (username: string, password: string): Promise<boolean> => {
      ...
      const SEED_HASH = '240be518fabd2724ddb6f04eeb1da5967448d7e831d729d2d0c97e51a9e95aaa'; // admin123
      const activeTeam = team.filter(t => !t.deleted);
      if (username === 'admin' && hashHex === SEED_HASH && activeTeam.length === 0) {
        setIsAuthenticated(true);
        setCurrentUser({ id: 'admin', name: 'Administrator', role: 'ADMIN', status: 'ACTIVE' } as TeamMember);
    ...
    return <Login onLogin={handleLogin} companyInfo={companyInfo} teamCount={team.length} />;
  }
  ...
  const isHardcodedView = [
    ...
    'QUALITY', 'STOCK_TRANSFER', 'STOCK_AUDIT', 'TAX_INVOICE',
    ...
    'TALLY_INTEGRATION', 'GALLERY', 'EMAIL_HUB'
                          ...
                          <>
                            ...
                            >
                              <div className="p-4 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center bg-slate-50/50 dark:bg-slate-900/50">
                                <div>
                                  <h3 className="text-sm font-bold text-slate-800 dark:text-white uppercase tracking-tight">Notifications</h3>
                                  <p className="text-[10px] text-slate-500 mt-0.5">{notifications.filter(n => !n.read).length} unread</p>
                                </div>
                                <div className="flex items-center gap-2">
                                  <button onClick={() => { handleMarkAllAsRead(); setIsNotificationDropdownOpen(false); }} className="text-[10px] font-bold text-indigo-600 hover:text-indigo-700 hover:bg-indigo-50 px-2 py-1 rounded transition-colors" title="Mark all as read">
                                    <Check className="w-4 h-4" />
                                  </button>
                                  <button onClick={() => setIsNotificationDropdownOpen(false)} className="text-slate-400 hover:text-slate-600 p-1 rounded-md hover:bg-slate-100 transition-colors">
                                    <X className="w-4 h-4" />
                                  </button>
                                </div>
                              </div>
                              ...
                              <div className="flex-1 overflow-y-auto custom-scrollbar p-2 space-y-1">
                                {notifications.length > 0 ? (
                                  notifications.slice(0, 50).map((n) => (
                                    <div
                                      ...
                                      className={`group relative p-3 rounded-xl border transition-all ${
                                        n.read ? 'bg-transparent border-transparent opacity-75' : 'bg-indigo-50/50 dark:bg-indigo-900/10 border-indigo-100 dark:border-indigo-900/30'
                                      }`}
                                    >
                                      <div className="flex gap-3">
                                        <div className={`p-2 rounded-lg border shrink-0 h-min ${
                                          ...
                                          {n.type === 'ERROR' && <XCircle className="w-3.5 h-3.5" />}
                                          {(!n.type || n.type === 'INFO') && <Info className="w-3.5 h-3.5" />}
                                        </div>
                                        <div className="flex-1 min-w-0">
                                          <div className="flex items-center justify-between gap-2 mb-0.5">
                                            <h4 className={`text-xs font-bold leading-tight uppercase tracking-tight truncate ${n.read ? 'text-slate-600 dark:text-slate-400' : 'text-slate-800 dark:text-white'}`}>
                                              {n.title}
                                            </h4>
                                          </div>
                                  ...
                                  <div className="py-12 flex flex-col items-center justify-center text-center">
                                    ...
                                    <p className="text-xs font-bold text-slate-500 uppercase tracking-wide">No Notifications</p>
                                  </div>
                                )}
                              </div>
                              <div className="p-3 border-t border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/50">
                                <button 
                                  onClick={() => { setCurrentView('NOTIFICATIONS'); setIsNotificationDropdownOpen(false); }}
                ...
                <div className={currentView === "DASHBOARD" ? "" : "max-w-[1500px] mx-auto px-4 py-5 lg:px-6 lg:py-6"}>
                    <AnimatePresence mode="wait">
                        ...
                        >
                            ...
                            {currentView === 'CUSTOMERS' && <Customers customers={active(customers)} onAdd={custMgr.add} onUpdate={custMgr.update} onDelete={custMgr.remove} currency={currencySymbol} />}
                            ...
                            {currentView === 'PACKING_SLIPS' && <PackingList slips={active(packingSlips)} orders={active(orders)} onAddSlip={packingSlipMgr.add} onUpdateSlip={packingSlipMgr.update} companyInfo={companyInfo} />}

                            {/* Production & Inventory */}
                            {currentView === 'PRODUCTION' && <Production jobs={active(production)} karigars={active(karigars)} designs={active(designs)} inventory={active(inventory)} machines={active(machines)} samples={active(samples)} orders={active(orders)} onAddJob={prodMgr.add} onUpdateJob={handleJobUpdate} onDeleteJob={(id) => prodMgr.remove(id)} onAddMachine={machineMgr.add} onUpdateMachine={machineMgr.update} onDeleteMachine={(id) => machineMgr.remove(id)} onAction={handleAction} onUpdateKarigar={karigarMgr.update} currency={currencySymbol} />}
                            ...
                            {currentView === 'QUALITY' && <QualityControl reports={active(qualityReports)} inspections={active(inspections)} jobs={active(production)} designs={active(designs)} inventory={active(inventory)} onAddReport={qualityMgr.add} onUpdateReport={qualityMgr.update} onAddInspection={inspectionMgr.add} currency={currencySymbol} geminiApiKey={securityConfig.geminiApiKey} />}
                            ...
                            {currentView === 'GATE_PASS' && <GatePassSystem gatePasses={active(gatePasses)} onAdd={gatePassMgr.add} onUpdate={gatePassMgr.update} />}
                            ...
                            {currentView === 'FABRIC_COSTING' && <FabricCostingWorkspace costings={active(fabricCostings)} designs={active(designs)} inventory={active(inventory)} yarnLots={active(yarnLots)} onAddCosting={costingMgr.add} onUpdateCosting={costingMgr.update} onDeleteCosting={costingMgr.remove} currency={currencySymbol} companyInfo={companyInfo} />}
                            {currentView === 'MARGIN_COSTING' && <MarginCostingEngine costings={active(marginCostings)} onAdd={marginCostingMgr.add} onUpdate={marginCostingMgr.update} />}
                            ...
                            {currentView === 'WASTE_MANAGEMENT' && <WasteManagement wasteLogs={active(wasteLogs)} onAdd={wasteMgr.add} onUpdate={wasteMgr.update} />}
                            {currentView === 'BROKERAGE' && <BrokerageTracker brokerLogs={active(brokerLogs)} onAdd={brokerMgr.add} onUpdate={brokerMgr.update} />}
                            ...
                            {currentView === 'CASH_BOOK' && <CashBook transactions={active(transactions)} customers={active(customers)} suppliers={active(suppliers)} team={active(team)} onAddTransaction={txnMgr.add} currency={currencySymbol} />}
                            ...
                            {currentView === 'EMAIL_HUB' && <EmailHub 
                            ...
                            {currentView === 'PROJECTS' && <Projects projects={active(projects)} team={active(team)} customers={active(customers)} onAddProject={projectMgr.add} onUpdateProject={projectMgr.update} onDeleteProject={projectMgr.remove} currency={currencySymbol} geminiApiKey={securityConfig.geminiApiKey} />}
