...
export interface Transaction extends BaseEntity { 
  ...
  description: string; 
  amount: number; 
  type: 'INCOME' | 'EXPENSE' | 'RECEIVE' | 'PAY' | 'INTERNAL_TRANSFER'; 
  category: string; 
...
export interface GatePass extends BaseEntity { number: string; date: string; type: 'INWARD' | 'OUTWARD' | 'RETURNABLE' | 'NON_RETURNABLE'; referenceId: string; vehicleNo?: string; driverName?: string; partyName?: string; status?: string; items?: { itemName: string; qty: number; unit: string; purpose?: string; }[]; }
