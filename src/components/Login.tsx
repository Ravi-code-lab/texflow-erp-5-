...
const ModeCard: React.FC<{
  ...
  >
    <div className="flex items-start gap-4">
      <div
        className={`p-3 rounded-xl ${accent} text-white shrink-0 group-hover:scale-110 transition-transform duration-200`}
      >
        {icon}
      </div>
      <div className="flex-1 min-w-0">
        ...
        <p
          className={`text-xs font-semibold uppercase tracking-widest mt-0.5 mb-2 ${accent.includes("indigo") ? "text-indigo-300" : "text-emerald-300"}`}
        >
          {subtitle}
        </p>
  ...
  const handleConnect = async () => {
    ...
    try {
      ...
      if (result.ok) {
        ...
        setErrorMsg(
          result.error ||
            "Cannot reach server. Make sure the server PC is running and on the same network.",
        );
      }
    } catch {
      setStatus("fail");
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center gap-3 mb-2">
        ...
        <div>
          ...
          <p className="text-indigo-200/60 text-xs">
            Enter the server PC's local IP address
          </p>
        </div>
      </div>
      ...
      <div className="p-3 rounded-xl bg-indigo-500/10 border border-indigo-400/20 text-xs text-indigo-200/80 leading-relaxed">
        ...
        On the <strong>Server PC</strong>, open Settings → Network — note the IP
        shown (e.g.{" "}
        <code className="bg-white/10 px-1 rounded">192.168.1.5</code>). Enter it
        below.
      </div>

      <div className="space-y-1">
        <label className="text-xs font-bold text-indigo-200 uppercase tracking-wider ml-1">
          Server IP Address
        </label>
      ...
      {status === "ok" && (
        <div className="p-3 bg-emerald-500/20 border border-emerald-500/50 rounded-xl text-emerald-200 text-sm flex items-center gap-2">
          <CheckCircle2 className="w-4 h-4 shrink-0" /> Connected! Loading
          login…
        </div>
      )}
...
const Login: React.FC<LoginProps> = ({
  ...
  return (
    <div className="min-h-screen relative flex items-center justify-center overflow-hidden bg-slate-900">
      ...
      <div
        className="absolute bottom-0 right-0 w-[500px] h-[500px] bg-purple-600/30 rounded-full blur-[120px] animate-pulse"
      ...
      <div className="relative z-10 w-full max-w-md p-8 bg-white/10 backdrop-blur-xl border border-white/20 rounded-3xl shadow-2xl">
        <div className="text-center mb-7">
          ...
          <p className="text-indigo-200 text-xs mt-1 font-medium tracking-widest uppercase">
        ...
        {step === "login" && (
          <div className="space-y-5">
            <div className="flex items-center justify-between mb-1">
              <button
                onClick={handleBack}
                className="flex items-center gap-1 text-xs text-indigo-300/60 hover:text-indigo-200 transition-colors"
              >
              ...
              <span
                className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[10px] font-bold uppercase tracking-widest ${
                  mode === "server" ||
                  (!isLanClientMode() && !(window as any).process?.type)
              ...
              >
                ...
                (!isLanClientMode() && !(window as any).process?.type) ? (
                  <>
                    <Server className="w-3 h-3" /> Standalone / Server
                  </>
                ) : (
                  <>
            ...
            {!(window as any).process?.type && !isLanClientMode() && (
              <div className="flex justify-end mt-[-10px] pb-3">
                <button
                  type="button"
                  onClick={() => {
                    setMode("client");
                  ...
                  className="text-[10px] font-bold uppercase tracking-widest text-emerald-400/80 hover:text-emerald-300 transition-colors flex items-center gap-1"
                >
                  <Wifi className="w-3 h-3" /> Connect to LAN Server
                </button>
              </div>
            )}
            ...
            {mode === "client" && (
              <div className="p-2.5 bg-emerald-500/10 border border-emerald-400/20 rounded-xl text-emerald-200/80 text-[11px] flex items-center gap-2">
                ...
                Connected to server — data syncs in real-time from the server
                PC.
              </div>
            )}
            ...
            {teamCount === 0 && mode === "server" && (
              <div className="p-3 bg-indigo-500/20 border border-indigo-400/40 rounded-xl text-indigo-100 text-xs text-center">
                ...
                First boot — use <strong>admin</strong> /{" "}
                <strong>admin123</strong> then create your team in Settings →
                Users.
              </div>
            )}
            ...
            <div className="mt-4 border-t border-white/10 pt-4">
              <p className="text-[10px] text-indigo-300/60 uppercase tracking-widest text-center mb-3">
                Access Levels
              </p>
              <div className="grid grid-cols-2 gap-1.5">
                {(Object.entries(ROLE_LABELS) as [UserRole, string][]).map(
                  ([role, label]) => (
                    ...
                    >
                      <div
                        className={`w-2 h-2 rounded-full bg-gradient-to-r ${ROLE_COLORS[role]} shrink-0`}
                      />
                      <div>
            ...
            <p className="mt-2 text-center text-xs text-indigo-300/40">
              Role-Based Access Control · SHA-256 Secured
            </p>
          </div>
        )}
      </div>
