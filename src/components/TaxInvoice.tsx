...
const TaxInvoice: React.FC<TaxInvoiceProps> = ({ 
  ...
  return (
    <div className="flex flex-col h-full bg-[#f4f5f6] font-sans antialiased text-[#1c2126] absolute inset-0 rounded-tl-xl overflow-hidden">
                                ...
                                <tbody>
                                   {formData.items.map((item, idx) => (
                                      <tr key={idx} className="border-b border-[#d1d8dd]/50 hover:bg-[#f4f5f6]/50">
                                         <td className="py-2 pl-3 font-semibold text-[#1c2126]">
                                            <div className="flex items-center gap-3">
      ...
      {selectedInvoice && <OrderDetailsModal order={selectedInvoice} customer={customers.find(c => c.name === selectedInvoice.customerName)} onClose={() => setSelectedInvoice(null)} currency={currency} designs={designs} inventory={inventory} />}
    </div>
  );
};
