    ...
    return production
      .filter(wo => {
        ...
        const hasOp = (wo.operations || []).some(op =>
          ((op.name||"").toLowerCase().includes(taskName.toLowerCase()) || (op.stage||"").toLowerCase().includes(taskName.toLowerCase()) || (op.workstationType||"").toLowerCase().includes(taskName.toLowerCase())) &&
...
export default function DeptTaskPage({ taskName, production, onUpdateWorkOrder, karigars }: Props) {
  ...
  const deptOps = useMemo(() =>
    production.flatMap(wo =>
      (wo.operations || [])
        .filter(op => ((op.name||"").toLowerCase().includes(taskName.toLowerCase()) || (op.stage||"").toLowerCase().includes(taskName.toLowerCase()) || (op.workstationType||"").toLowerCase().includes(taskName.toLowerCase())))
        .map((op, idx) => ({
          ...op,
          woId: wo.id,
