...
const PackingList: React.FC<PackingSlipProps> = ({ 
  slips, orders, onAddSlip, onUpdateSlip 
}) => {
  ...
  const generatePDF = (slip: any) => {
    ...
    doc.text("PACKING SLIP", 15, 20);
    
    doc.setFontSize(9);
    doc.setFont("helvetica", "normal");
    doc.setTextColor(100);
    doc.text(`Slip ID: ${slip.id}`, 15, 27);
    doc.text(`Date: ${slip.date}`, 15, 32);

    doc.setTextColor(0);
    doc.setFont("helvetica", "bold");
    doc.text("DELIVERY DETAILS", pageWidth - 15, 20, { align: 'right' });
    doc.setFont("helvetica", "normal");
    doc.text(`Delivery Note: ${slip.deliveryNoteId}`, pageWidth - 15, 25, { align: 'right' });
    doc.text(`Package No: ${slip.fromPackageNo} to ${slip.toPackageNo}`, pageWidth - 15, 30, { align: 'right' });
    ...
    doc.setFont("helvetica", "normal");
    doc.text(slip.customerName || '', 15, 55);

    doc.setFont("helvetica", "bold");
    doc.text("Weight Information:", 120, 50);
    doc.setFont("helvetica", "normal");
    doc.text(`Net: ${slip.netWeight || 0} kg | Gross: ${slip.grossWeight || 0} kg`, 120, 55);
