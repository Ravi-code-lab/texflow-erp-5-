...
const Accounting: React.FC<AccountingProps> = ({
  ...
  const [journalRows, setJournalRows] = useState<{id: string; account: string; accountType: 'DEBIT' | 'CREDIT'; amount: number}[]>([
    { id: '1', account: '', accountType: 'DEBIT', amount: 0 },
    { id: '2', account: '', accountType: 'CREDIT', amount: 0 }
  ]);
  ...
  const handleAddJournal = useCallback((e: React.FormEvent) => {
    ...
    setJournalForm({ date: today(), narration: '', voucherType: 'JOURNAL' });
    setJournalRows([
      { id: '1', account: '', accountType: 'DEBIT', amount: 0 },
      { id: '2', account: '', accountType: 'CREDIT', amount: 0 }
    ]);
  }, [journalForm, journalRows, onAddTransaction]);
    ...
    <div className="flex flex-col h-full bg-slate-50 dark:bg-slate-950 animate-fade-in font-sans">
      ...
      <BaseModal isOpen={isJournalModalOpen} onClose={() => setIsJournalModalOpen(false)} title="New Journal Voucher" size="md">
        <form onSubmit={handleAddJournal} className="space-y-4">
          ...
          <div className="bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl overflow-hidden mt-4">
            <div className="px-4 py-2 bg-slate-100 dark:bg-slate-800 flex items-center justify-between border-b border-slate-200 dark:border-slate-700">
                ...
                <div key={row.id} className="p-3">
                  <div className="grid grid-cols-12 gap-3 items-center">
                    <div className="col-span-3">
                      <select value={row.accountType} onChange={e => {
                        const nl = [...journalRows]; nl[index].accountType = e.target.value as 'DEBIT'|'CREDIT'; setJournalRows(nl);
                      }} className="w-full text-xs border border-slate-200 dark:border-slate-700 rounded-lg px-2 py-2 bg-white dark:bg-slate-800 outline-none focus:ring-1 focus:ring-indigo-500 font-bold uppercase text-slate-500">
                        <option value="DEBIT">Dr (Debit)</option>
                        <option value="CREDIT">Cr (Credit)</option>
                      </select>
                    </div>
                    <div className="col-span-5">
                      <input type="text" value={row.account} onChange={e => {
                        const nl = [...journalRows]; nl[index].account = e.target.value; setJournalRows(nl);
                      }} className="w-full text-sm border border-slate-200 dark:border-slate-700 rounded-lg px-3 py-1.5 bg-white dark:bg-slate-800 outline-none focus:ring-1 focus:ring-indigo-500" placeholder="e.g. Sales, Bank..." />
                    </div>
                    <div className="col-span-3">
                      <input type="number" step="0.01" value={row.amount || ''} onChange={e => {
                        const nl = [...journalRows]; nl[index].amount = parseFloat(e.target.value) || 0; setJournalRows(nl);
                      }} className="w-full text-sm font-mono border border-slate-200 dark:border-slate-700 rounded-lg px-3 py-1.5 bg-white dark:bg-slate-800 outline-none focus:ring-1 focus:ring-indigo-500 text-right" placeholder="0.00" />
                    </div>
                    <div className="col-span-1 text-center">
                      {journalRows.length > 2 && (
                        <button type="button" onClick={() => setJournalRows(journalRows.filter(r => r.id !== row.id))} className="text-[10px] uppercase font-bold text-rose-500 hover:text-rose-600">Del</button>
                      )}
                    </div>
                  </div>
                </div>
            ...
            <div className="px-3 py-2 bg-slate-100 dark:bg-slate-800 border-t border-slate-200 dark:border-slate-700 flex justify-between items-center">
              <button type="button" onClick={() => setJournalRows([...journalRows, { id: Date.now().toString(), account: '', accountType: 'DEBIT', amount: 0 }])} className="text-[10px] font-bold uppercase text-indigo-600 hover:text-indigo-700">
                + Add Row
              </button>
              <div className="flex gap-4 font-mono font-bold text-sm">
                <span className="text-blue-600">Dr {journalRows.filter(r => r.accountType === 'DEBIT').reduce((s,r) => s+r.amount, 0).toLocaleString()}</span>
                <span className="text-orange-600">Cr {journalRows.filter(r => r.accountType === 'CREDIT').reduce((s,r) => s+r.amount, 0).toLocaleString()}</span>
              </div>
            </div>
          </div>
