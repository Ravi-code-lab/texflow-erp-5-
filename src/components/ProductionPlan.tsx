...
import { Order, Design, ProductionJob, OrderItem } from '../types';
...
export default function ProductionPlan({ orders, designs, jobs, onAction }: ProductionPlanProps) {
  ...
  const handleCreatePlan = () => {
     setActivePlan({
       ...
       items: pendingOrders.flatMap(o => o.items.map((i: OrderItem) => ({
         orderId: o.id,
                 ...
                 {pendingOrders.map(o => (
                   <div key={o.id} className="p-3 border border-[#d1d8dd] rounded bg-[#fdfdfd] flex justify-between items-center text-sm">
                      ...
                      <div className="text-right">
                         ...
                         <p className="font-medium text-[#1c2126] mt-1">{o.items.reduce((s: number, i: OrderItem) => s + i.quantity, 0)} Units</p>
                      </div>
                   </div>
                 ))}
              </div>
