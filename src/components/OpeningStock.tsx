...
const ReadyStock: React.FC<OpeningStockProps> = ({
  ...
  const handleSave = (e: React.FormEvent) => {
    ...
    const item = {
      ...formData,
      quantity,
      id:        editingId || `RS-${Date.now().toString().slice(-6)}`,
      doctype:   READY_STOCK_DOCTYPE,
                      ...
                      {IS_FABRIC_CATEGORY(item.type) && item.fabricMetering && item.fabricMetering.length > 0 && (
                        <div className="flex flex-wrap gap-1.5">
                          {item.fabricMetering.map((e: FabricMeteringEntry, idx: number) => (
                            <div key={idx} className="flex items-center gap-1 bg-indigo-50 dark:bg-indigo-900/20 border border-indigo-100 dark:border-indigo-800 rounded px-2 py-0.5">
              ...
              <div className="md:col-span-2">
                ...
                <div className="flex gap-2 flex-wrap">
                  {(formData.imageUrls || []).map((img, idx) => (
                    <div key={idx} className="relative w-16 h-16 rounded-lg overflow-hidden border-2 border-slate-200 dark:border-slate-700 group">
                      <img src={img} alt="" className="w-full h-full object-cover" referrerPolicy="no-referrer" />
                      <button
                        type="button"
                    ...
                    {(formData.fabricMetering || []).length === 0 ? (
                      ...
                      <div className="space-y-2">
                        {(formData.fabricMetering || []).map((entry, idx) => (
                          <div key={idx} className="grid grid-cols-12 gap-2 items-end bg-indigo-50/50 dark:bg-indigo-900/10 border border-indigo-100 dark:border-indigo-900 rounded-lg p-2">
                            {/* Direction */}
