...
const OpeningStock: React.FC<OpeningStockProps> = ({ 
  ...
  return (
    <div className="space-y-6 h-full flex flex-col bg-[#f8fafc] dark:bg-slate-950 -mx-4 -my-5 px-4 py-5 lg:-m-6 lg:p-6 animate-fade-in font-sans">
                      ...
                      <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                          {filteredItems.map(item => (
                              <tr key={item.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-colors group">
                                  <td className="p-4">
                                      <div className="flex items-center gap-3">
                                        <div className="w-9 h-9 rounded-lg overflow-hidden border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 flex items-center justify-center shrink-0">
                                          {item.imageUrl
          ...
          <form onSubmit={handleSave} className="space-y-6 pb-20">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="md:col-span-2">
                      ...
                      <div className="flex items-center gap-3">
                        <div className="w-14 h-14 rounded-xl overflow-hidden border-2 border-dashed border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 flex items-center justify-center shrink-0 cursor-pointer relative group">
                          {formData.imageUrl
                            ? <img src={formData.imageUrl} alt="" className="w-full h-full object-cover" referrerPolicy="no-referrer" />
                            : <Package className="w-5 h-5 text-slate-300" />
                          }
                          <input type="file" accept="image/*" className="absolute inset-0 opacity-0 cursor-pointer" title="Upload photo" onChange={async e => {
                            const f = e.target.files?.[0];
                            if (f) { const url = await commitImage(f, 500); setFormData(d => ({...d, imageUrl: url})); }
                          }} />
                        </div>
                        <input required className="flex-1 border border-slate-200 dark:border-slate-700 rounded-lg p-2.5 text-sm font-bold uppercase bg-white dark:bg-slate-800 outline-none focus:ring-1 focus:ring-indigo-500" value={formData.name || ''} onChange={e => setFormData({...formData, name: e.target.value.toUpperCase()})} placeholder="e.g. COTTON 40S COMBED YARN" />
                      </div>
                      <p className="text-[10px] text-slate-400 mt-1">Click the box to add a photo for easy identification</p>
                  </div>
