...
const DEFAULT_MEASUREMENT_POINTS = ['Chest', 'Length', 'Shoulder', 'Sleeve', 'Waist'];

const QualityControl: React.FC<QualityControlProps> = ({ reports, inspections = [], jobs = [], designs = [], inventory = [], onAddReport, onUpdateReport, onAddInspection, currency = '₹', geminiApiKey }) => {
  ...
  const [reportData, setReportData] = useState<Partial<QualityReport>>({
    date: new Date().toISOString().split('T')[0],
    defects: [],
    measurements: DEFAULT_MEASUREMENT_POINTS.map(point => ({ point, target: 0, tolerance: 0.5, actual: 0, unit: 'IN', passed: true }))
  });

  const selectedJob = useMemo(
    () => jobs.find(job => job.id === reportData.jobId),
    [jobs, reportData.jobId]
  );

  const selectedJobDesign = useMemo(
    () => designs.find(design => design.name === selectedJob?.productName || design.sku === selectedJob?.styleCode),
    [designs, selectedJob]
  );

  const reportProductImage = reportData.imageUrl || selectedJob?.imageUrl || resolveProductImage(selectedJob?.productName || reportData.productName || '', designs, inventory, selectedJob?.styleCode || reportData.styleCode || '');

  const updateMeasurement = (index: number, updates: Partial<GarmentMeasurementCheck>) => {
    const nextMeasurements = [...(reportData.measurements || [])];
    const next = { ...nextMeasurements[index], ...updates };
    next.passed = Math.abs((Number(next.actual) || 0) - (Number(next.target) || 0)) <= (Number(next.tolerance) || 0);
    nextMeasurements[index] = next;
    setReportData({ ...reportData, measurements: nextMeasurements });
  };
  ...
  const handleSaveReport = (e: React.FormEvent) => {
     ...
     const newReport = {
        ...
        color: selectedJob?.color || reportData.color,
        imageUrl: reportProductImage,
     } as QualityReport;
    ...
    <div className="space-y-4 h-full flex flex-col max-w-[1400px] mx-auto pb-6">
      ...
      <div className="flex flex-col md:flex-row justify-between items-center gap-4 bg-white dark:bg-slate-800 p-2 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm shrink-0">
          ...
          {activeTab === 'GARMENT_QC' && (
            <button onClick={() => { setReportData({ date: new Date().toISOString().split('T')[0], defects: [], measurements: DEFAULT_MEASUREMENT_POINTS.map(point => ({ point, target: 0, tolerance: 0.5, actual: 0, unit: 'IN', passed: true })) }); setIsReportModalOpen(true); }} className="bg-indigo-600 hover:bg-indigo-700 text-white px-4 py-2 rounded-lg text-xs font-bold flex items-center gap-2 transition-all shadow-lg active:scale-95">
                  ...
                  <div className="xl:col-span-2 bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 h-full flex flex-col shadow-sm overflow-hidden">
                      <div className="overflow-x-auto flex-1 custom-scrollbar">
                          ...
                          <thead className="bg-slate-50 dark:bg-slate-900 text-slate-500 sticky top-0 z-10 border-b border-slate-200 dark:border-slate-700 uppercase font-bold tracking-widest">
                              <tr>
                                  <th className="px-5 py-4">Style / Job Node</th>
                                  ...
                                  <tr key={report.id} className="hover:bg-slate-50/80 dark:hover:bg-slate-700/30 transition-colors group">
                                      <td className="px-5 py-3">
                                          <div className="flex items-center gap-3">
                                              <ProductImageThumb productName={report.productName || report.jobId} sku={report.styleCode} designs={designs} inventory={inventory} size="sm" />
                                              <div>
                                                  <p className="font-mono font-bold text-indigo-600">{report.jobId}</p>
                                                  <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{report.styleCode || report.productName || 'Style pending'} {report.color ? `/ ${report.color}` : ''}</p>
                                              </div>
                                          </div>
                                      </td>
      ...
      <BaseModal isOpen={isReportModalOpen} onClose={() => setIsReportModalOpen(false)} title="New Garment QC Check" size="md">
        <form onSubmit={(e) => {
             e.preventDefault();
             if (!reportData.jobId) return;
             const failedMeasurements = (reportData.measurements || []).filter(item => !item.passed).length;
             ...
             const newReport = {
                ...
                imageUrl: reportProductImage,
                defectsFound,
                defectType: failedMeasurements > 0 ? 'MEASUREMENT' : (reportData.defectType || 'OTHER'),
                status: defectsFound === 0 ? 'PASSED' : defectsFound <= 2 ? 'RE-WORK' : 'FAILED'
             } as QualityReport;
             if (reportData.id && onUpdateReport) onUpdateReport(newReport);
             else if (onAddReport) onAddReport(newReport);
             setIsReportModalOpen(false);
          }} className="space-y-6">
           <div className="grid grid-cols-[88px_1fr] gap-4 items-center p-4 bg-slate-50 rounded-2xl border border-slate-200">
              <ProductImageThumb productName={selectedJob?.productName || reportData.productName || ''} sku={selectedJob?.styleCode || reportData.styleCode} designs={designs} inventory={inventory} size="lg" />
              <div className="min-w-0">
                <p className="text-[10px] font-black text-indigo-600 uppercase tracking-widest">Garment QC Reference</p>
                <h3 className="text-sm font-black text-slate-800 truncate">{selectedJob?.productName || reportData.productName || 'Select Work Order'}</h3>
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1">{selectedJob?.styleCode || reportData.styleCode || 'Style pending'} {selectedJob?.color ? `/ ${selectedJob.color}` : ''}</p>
              </div>
           </div>

           <div className="space-y-1">
               <label className="text-xs font-bold text-slate-500 uppercase">Work Order</label>
               <select required value={reportData.jobId || ''} onChange={e => {
                  ...
                  setReportData({
                    ...
                    color: job?.color,
                    checkedQuantity: job?.quantity || reportData.checkedQuantity,
                    operationId: job?.operations?.find(operation => operation.status !== 'COMPLETED')?.id || job?.operations?.[0]?.id,
                    imageUrl: job?.imageUrl || resolveProductImage(job?.productName || '', designs, inventory, job?.styleCode || ''),
                  });
                  ...
                  {jobs.map(job => <option key={job.id} value={job.id}>{job.id} - {job.productName} {job.styleCode ? `(${job.styleCode})` : ''}</option>)}
               </select>
           </div>
           ...
           <div className="grid grid-cols-2 gap-4">
             <div className="space-y-1">
               ...
               <select value={reportData.operationId || ''} onChange={e => setReportData({...reportData, operationId: e.target.value})} className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-all">
                  <option value="">General inspection</option>
                  {(selectedJob?.operations || []).map(operation => <option key={operation.id} value={operation.id}>{operation.name}</option>)}
               </select>
             </div>
             <div className="space-y-1">
               <label className="text-xs font-bold text-slate-500 uppercase">Size</label>
               <select value={reportData.size || ''} onChange={e => setReportData({...reportData, size: e.target.value})} className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-all">
                  <option value="">All sizes</option>
                  {Object.keys(selectedJob?.sizeWise || {}).map(size => <option key={size} value={size}>{size}</option>)}
               </select>
             </div>
           </div>
           ...
           <div className="space-y-1">
               ...
               <select value={reportData.defectType || ''} onChange={e => setReportData({...reportData, defectType: e.target.value as any})} className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-all">
                  ...
                  <option value="MEASUREMENT">Measurement Out</option>
                  <option value="STITCHING">Stitching Defect</option>
                  <option value="PACKING">Packing Issue</option>
           ...
           <div className="space-y-3 p-4 bg-slate-50 rounded-2xl border border-slate-200">
               <div className="flex items-center justify-between">
                 <label className="text-xs font-bold text-slate-500 uppercase">Measurement Checks</label>
                 <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Tolerance in inches</span>
               </div>
               <div className="space-y-2">
                 {(reportData.measurements || []).map((measurement, index) => (
                   <div key={`${measurement.point}-${index}`} className="grid grid-cols-[1fr_70px_70px_70px_28px] gap-2 items-center">
                     <input className="px-2 py-1.5 bg-white border border-slate-200 rounded text-xs font-bold" value={measurement.point} onChange={e => updateMeasurement(index, { point: e.target.value })} />
                     <input type="number" className="px-2 py-1.5 bg-white border border-slate-200 rounded text-xs" value={measurement.target || ''} onChange={e => updateMeasurement(index, { target: Number(e.target.value) })} placeholder="Target" />
                     <input type="number" className="px-2 py-1.5 bg-white border border-slate-200 rounded text-xs" value={measurement.actual || ''} onChange={e => updateMeasurement(index, { actual: Number(e.target.value) })} placeholder="Actual" />
                     <input type="number" className="px-2 py-1.5 bg-white border border-slate-200 rounded text-xs" value={measurement.tolerance || ''} onChange={e => updateMeasurement(index, { tolerance: Number(e.target.value) })} placeholder="+/-" />
                     <div className={`w-6 h-6 rounded-full flex items-center justify-center ${measurement.passed ? 'bg-emerald-100 text-emerald-600' : 'bg-rose-100 text-rose-600'}`}>
                       {measurement.passed ? <CheckCircle className="w-3.5 h-3.5" /> : <XCircle className="w-3.5 h-3.5" />}
                     </div>
                   </div>
                 ))}
               </div>
