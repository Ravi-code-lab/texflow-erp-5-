...
export const WorkOrderFinancials: React.FC<WorkOrderFinancialsProps> = ({
  ...
  const laborCost = costingSummary?.laborCost ?? operations.reduce((sum, op) => {
    const rate = op.ratePerPiece ?? op.rate ?? 0;
    return sum + (op.rateUnit === 'PER_HOUR' ? rate * (op.plannedHours ?? 1) : rate * (quantity || 1));
  }, 0);
            ...
            {operations.map((op, i) => (
              <div key={i} className="flex justify-between text-xs">
                ...
                <span className="font-bold text-slate-700 font-mono tabular-nums">
                  {currency}{(() => {
                    const rate = op.ratePerPiece ?? op.rate ?? 0;
                    return (op.rateUnit === 'PER_HOUR' ? rate * (op.plannedHours ?? 1) : rate * (quantity || 1)).toLocaleString();
                  })()}
                </span>
              </div>
            ))}
