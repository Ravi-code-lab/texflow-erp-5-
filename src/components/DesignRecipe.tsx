...
const DesignRecipe: React.FC<DesignRecipeProps> = ({ 
  ...
  const getSubAssemblyCost = (name: string): number => {
  ...
  const calculatedCosting = useMemo(() => {
    ...
    // 2. Routing Operations Costs (ERPNext timesheet-based calculation)
    const operationCost = (formData.operations || []).reduce((acc, op) => {
      const perUnitCost = (op.cycleTimeMinutes / 60) * op.hourlyRate;
  ...
  }, [formData.recipe, formData.operations, formData.scrapItems, formData.processLossPercent, formData.overheadCharges, inventory]);
  ...
  const handleSave = (e: React.FormEvent) => {
    ...
    const baseUnitItem: Design = { 
      ...
      id: editingId || `BOM-${Date.now().toString().slice(-4)}`, 
                ...
                {activeTab === 'OPERATIONS' && (
                   <div className="space-y-4">
                       <div className="bg-slate-50 p-4 border border-dashed border-slate-200 rounded flex gap-2 items-end">
                            ...
                            <div className="w-24 space-y-1">
                                <label className="text-[10px] text-slate-500 font-bold uppercase">Cost/Hour</label>
                                <input 
                                  type="number" 
                                  className="w-full px-2 py-1.5 bg-white border border-slate-300 rounded text-xs text-center font-semibold"
                                  placeholder="150 / hr" 
                                  value={newOperation.hourlyRate || ''}
                                  onChange={e => setNewOperation({...newOperation, hourlyRate: Number(e.target.value)})}
                                />
                            </div>
                            <button 
                              type="button"
                              ...
                              <table className="w-full text-left border-collapse">
                                 <thead>
                                    <tr className="bg-slate-50 text-[10px] text-slate-500 font-bold uppercase border-b border-slate-200">
                                       ...
                                       <th className="py-2.5 px-3 text-center w-24">Cycle Time</th>
                                       <th className="py-2.5 px-3 text-right w-32">Operating Hourly Rate</th>
                                 ...
                                 <tbody className="divide-y divide-slate-100">
                                    {formData.operations.map((op, idx) => {
                                       const unitOpCost = (op.cycleTimeMinutes / 60) * op.hourlyRate;
                                       return (
                                          <tr key={idx} className="hover:bg-slate-50/50 text-xs">
                                             ...
                                             <td className="py-2 px-3 text-right text-slate-500 font-medium tabular-nums">{currency}{op.hourlyRate}/Hr</td>
