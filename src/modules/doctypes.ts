...
export interface DocField {
  fieldname: string;
  label: string;
  fieldtype: 'Data' | 'Int' | 'Float' | 'Currency' | 'Date' | 'Datetime' | 'Select' | 'Check' | 'Text' | 'Small Text' | 'Link' | 'Table' | 'Attach' | 'Read Only';
  reqd?: boolean;
...
export const DOCTYPE_SCHEMAS: DocTypeSchema[] = [
  {
    ...
    titleField: 'customer',
    statusField: 'status',
    statuses: ['Draft', 'Confirmed', 'In Production', 'Dispatched', 'Closed', 'Cancelled'],
    isSubmittable: true,
    fields: [
      { fieldname: 'name', label: 'Order ID', fieldtype: 'Data', required: true },
      { fieldname: 'customer', label: 'Customer', fieldtype: 'Link', linkTo: 'Customer', required: true },
      { fieldname: 'date', label: 'Date', fieldtype: 'Date', required: true },
      { fieldname: 'status', label: 'Status', fieldtype: 'Select', options: ['Draft','Confirmed','In Production','Dispatched','Closed','Cancelled'], default: 'Draft' },
  ...
  },
  {
    ...
    fields: [
      { fieldname: 'itemCode', label: 'Item Code', fieldtype: 'Data', required: true },
      { fieldname: 'itemName', label: 'Item Name', fieldtype: 'Data', required: true },
      { fieldname: 'category', label: 'Category', fieldtype: 'Select', options: ['Fabric','Yarn','Accessory','Finished Goods','Packaging'] },
      { fieldname: 'unit', label: 'Unit', fieldtype: 'Select', options: ['KG','METER','PIECE','LITER','YARD'] },
      { fieldname: 'currentStock', label: 'Current Stock', fieldtype: 'Float' },
      { fieldname: 'reorderLevel', label: 'Reorder Level', fieldtype: 'Float' },
      { fieldname: 'status', label: 'Status', fieldtype: 'Select', options: ['Active','Inactive','Discontinued'], default: 'Active' },
    ],
  },
  {
    view: 'SUPPLIERS',
  ...
  },
  {
    ...
    isSubmittable: true,
    fields: [
      { fieldname: 'supplier', label: 'Supplier', fieldtype: 'Link', linkTo: 'Supplier', required: true },
      { fieldname: 'date', label: 'Date', fieldtype: 'Date', required: true },
      { fieldname: 'status', label: 'Status', fieldtype: 'Select', options: ['Draft','Ordered','Received','Closed','Cancelled'], default: 'Draft' },
      { fieldname: 'total', label: 'Total Amount', fieldtype: 'Currency' },
    ],
  },
  {
    ...
    statusField: 'status',
    statuses: ['Draft', 'In Progress', 'Quality Check', 'Completed', 'Cancelled'],
    fields: [
      { fieldname: 'productName', label: 'Product', fieldtype: 'Data', required: true },
      { fieldname: 'quantity', label: 'Quantity', fieldtype: 'Int', required: true },
      { fieldname: 'status', label: 'Status', fieldtype: 'Select', options: ['Draft','In Progress','Quality Check','Completed','Cancelled'], default: 'Draft' },
  ...
  },
  {
    ...
    fields: [
      { fieldname: 'purpose', label: 'Purpose', fieldtype: 'Select', options: ['Purchase','Transfer','Manufacture'], required: true },
      { fieldname: 'status', label: 'Status', fieldtype: 'Select', options: ['Draft','Pending','Ordered','Received','Cancelled'], default: 'Draft' },
...
export function getDocTypeSchema(view: ViewState): DocTypeSchema | undefined {
  return customSchemas.get(view) || DOCTYPE_SCHEMAS.find((s) => s.view === view);
}
